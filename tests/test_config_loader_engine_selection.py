...
class TestConfigLoaderEngineSelection(unittest.TestCase):
    ...
    def test_deepseek_v4_pro_non_ascend_does_not_enter_adapted_scope(self):
        ...
        params = {
            ...
            "device_count": 16,
            "engine_config": {
                "served_model_name": "DeepSeek-V4-Pro-w4a8-mtp",
                "ascend_platform": "a3",
            },
        }
    ...
    def test_pd_external_lb_topology_overrides_standard_parallel_env(self):
        """PD topology must win over standard TENSOR/DATA_PARALLEL_SIZE env values."""
        with tempfile.TemporaryDirectory() as tmpdir:
            model_dir = Path(tmpdir)
            (model_dir / "config.json").write_text(
                json.dumps({"architectures": ["Qwen3MoeForCausalLM"]}),
        ...
        engine_config = merged["engine_config"]
        self.assertEqual(engine_config.get("tensor_parallel_size"), 2)
        self.assertEqual(engine_config.get("data_parallel_size"), 2)
        kv_config = json.loads(engine_config["kv_transfer_config"])
        self.assertEqual(kv_config["kv_connector_extra_config"]["prefill"]["tp_size"], 2)
        self.assertEqual(kv_config["kv_connector_extra_config"]["prefill"]["dp_size"], 2)
