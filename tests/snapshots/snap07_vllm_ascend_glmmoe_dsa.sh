...
echo '[wings-cmd] >>> exec python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 4096 --tool-call-parser glm47 --reasoning-parser glm45 --quantization ascend --seed 1024 --max-num-seqs 8 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --enable-chunked-prefill --enable-prefix-caching --async-scheduling --additional-config '"'"'{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}'"'"' --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --host 10.0.0.1 --port 17000 --served-model-name GLM-Z1-Air --model /models/GLM-Z1-Air --dtype auto --kv-cache-dtype auto --block-size 128 --enable-auto-tool-choice --tensor-parallel-size 8'
python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 4096 --tool-call-parser glm47 --reasoning-parser glm45 --quantization ascend --seed 1024 --max-num-seqs 8 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --enable-chunked-prefill --enable-prefix-caching --async-scheduling --additional-config '{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}' --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --host 10.0.0.1 --port 17000 --served-model-name GLM-Z1-Air --model /models/GLM-Z1-Air --dtype auto --kv-cache-dtype auto --block-size 128 --enable-auto-tool-choice --tensor-parallel-size 8 &
ENGINE_PID=$!
