...
> 目标：证明 `feat/smart-three-feature-enablement`（`8e607d4`）相对 `master`（`0a8b3f9`）**只产生需求文档列明的预期变更，未丢失/未破坏任何既有功能**。
...
| **D6** | KV 卸载 auto 容量反向预算（C4，新增能力） | 4bed278 | auto 模式 branch 计算并写回 `LMCACHE_MAX_LOCAL_CPU_SIZE=<均卡>`/`cpu_swap_space_gb` + 强制 `--swap-space 0`；master 无此自动计算 |
...
master worktree (0a8b3f9) ─┐                          ┌─ 归一化(去临时路径churn + [wings-cmd]截断预览)
                           ├─ 同一场景 → 各自生产代码 ─┤
branch worktree (8e607d4) ─┘   生成 start_command.sh   └─ 逐行 diff → 按 D1–D5 分类 → 回归计数
```
...
4. **分类**：对每条差异行匹配 D1–D5 正则；不匹配即记入「疑似回归」清单（含场景名 + 原始行），供人工复核。

---

## 四、对比测试用例矩阵（详细 · 完备）

> 三特性开关一律经 **env**（orchestration_env：`ENABLE_SPECULATIVE_DECODE/ENABLE_SPARSE/LMCACHE_OFFLOAD`），`user_cli` 不含特性 CLI 标志。
...
- **覆盖**：4 引擎 × 12+ 架构 × 3 平台 × 单/双/PD 拓扑 × 7 特性组合 + 7 回归专项 + 2 边界 ≈ **45+ 对比场景**。
