...
LMCache             LMCACHE_MAX_LOCAL_CPU_SIZE              = M_offload ÷ N_card   ← 除卡数 → 均卡(per-card)
...
- **两路卸载复用 M_offload，仅末端单位不同 `🆕`**：`M_offload` 是**本节点总额**，LMCache 与 native/CPUOffloading 两后端**复用同一公式**算出它，差别只在落地：
  - **LMCache（`LMCACHE_MAX_LOCAL_CPU_SIZE`）÷ 卡数 → 均卡(per-card)**：LMCache 每 rank 自带一个 CPU 池，需要的是**单卡**额度，故写回 `M_offload ÷ N_card`。
...
flowchart TD
  ...
  G -- 是 --> H["熔断：不开 offload"]
  G -- 否 --> I{"哪种后端?"}
  I -- LMCache --> I1["写回 LMCACHE_MAX_LOCAL_CPU_SIZE<br/>= M_offload ÷ N_card（均卡/per-card）"]
  I -- native/cpuoffload --> I2["cpu_swap_space_gb / --kv_offloading_size<br/>= M_offload（不除卡数/整节点）"]
```
> 决策要点：auto 才算；引擎自耗按 `7G×(TP×DP)+3G` 线性估（不查表/不读 graph）；低于下限熔断；**两路复用 M_offload，LMCache ÷ 卡数、native 不除**。
...
| `deepseek_mtp` | `engine=vllm, model=glm-5`（基座），`ENABLE_SPECULATIVE_DECODE=true`，**不设** LMCACHE_OFFLOAD | deepseek_mtp | 需 spec∈白名单（glm-5 基座 ✓；注：glm-5.1 已收窄 sparse-only→落 suffix 地板） |
...
| `native_kv_offloading_backend` | `engine=vllm, model=deepseek-v4-flash`, `LMCACHE_OFFLOAD=true` | native `--kv_offloading_backend`，与 MTP 共存（§6-④ 已闭环，白名单已补行 `{spec,sparse,offload}`） |
| `disabled` | `engine=vllm, model=glm-5.1`, `LMCACHE_OFFLOAD=true` | ⚠ C14 已先关（glm-5.1·NV 白名单无 offload）→ `_build_cache_env_commands` 的 GLM51·NV 守卫变二级冗余 |
