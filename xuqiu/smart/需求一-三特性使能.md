...
M_engine  = Σ_本节点worker  w(model, quant, TP, graph)        ← 实测 PSS 查表
...
- **per-worker 走实测查表**，key＝`(模型, 量化档, TP, graph模式)`：graph 是主导变量（full graph 较 FULL_DECODE_ONLY +90~124%，w4a8 例外几乎不变），单常数会被击穿（MiniMax-M2.5 TP16 full graph 单 worker 6219 MB > 6G）。**miss（新模型）取该 TP 档 full-graph 上沿（现 ~6.5G/worker），单边保守宁可池小不可 OOM。**
...
> | 未决项 | 卡住 |
> | --- | --- |
> | `w(model,quant,TP,graph)` 实测 PSS 表 —— 含 **worker 计数口径**（DP2 行是否按 DP×TP） | 表准确性 + `C_fixed` 拟合 |
> | `C_fixed` 的 DP≥4 外推（现仅 DP1/DP2 两点拟合） | 高 DP 部署 |
> | **graph 三态如何在 wings 端判定** —— 现仅 `need_enforce_eager()` 二态（vllm_adapter.py:195），缺 full vs FULL_DECODE_ONLY | 查表 key 可计算性 |
