...
| 3 | KV卸载 auto 容量：容器内存 → 可用 offload **反向预算公式**(PSS 非 RSS / swap=0 / 查表 / 熔断) | §3.0 C4 |
...
| 4 | C4 实测 PSS 表 + worker 计数口径 / 权重挂载 NFS / `f_margin`·熔断下限 | 运维·实测给值，先保守兜底 | §3.0 待运维表 |
...
| 模式 | 触发 |
| --- | --- |
| `auto` | `LMCACHE_MAX_LOCAL_CPU_SIZE=auto`（C4 自算，§3.0） |
| `custom` | 给具体 GB 透传 |
...
```mermaid
flowchart TD
  A["_build_kv_sparse_cmd"] --> S["🆕 §2.4 入口读 SMART_SPARSE_LEVEL<br/>Performance 未实现→告警回落 Accuracy"]
...
```mermaid
flowchart TD
  A["resolve_offload_cpu_capacity_gb"] --> B{"CPU_SIZE 空或 'auto'<br/>且 LMCACHE_POD_MEMORY 非空?"}
  B -- 否（custom） --> C["透传具体 GB（不计算）"]
  B -- 是（auto） --> D["强制 swap_space=0"]
  D --> E["M_engine = Σ worker_PSS[模型,quant,TP] + C_fixed(DP)<br/>（graph 固定 full，§0 裁定3）"]
  E --> F["M_offload = (M_node − M_engine)×(1 − f_margin)"]
  F --> G{"M_offload < M_offload_min（如100G）?"}
  G -- 是 --> H["熔断：不开 offload"]
  G -- 否 --> I["写回 LMCACHE_MAX_LOCAL_CPU_SIZE"]
```
> 决策要点：auto 才算；PSS 查表 graph 取最坏档 full；低于下限熔断。
...
flowchart LR
  subgraph IN["入参"]
    direction TB
    I1["页面开关 enable*（默认全开）"]
    I2["env：LMCACHE_*/SMART_SPARSE_LEVEL/PD_ROLE/LMCACHE_POD_MEMORY"]
...
| --- | --- | --- |
| `lmcache_cpu+auto` | `model=glm-4.7` + `LMCACHE_MAX_LOCAL_CPU_SIZE=auto` + `LMCACHE_POD_MEMORY=<本节点GiB>` | LMCache CPU，C4 自算容量 |
