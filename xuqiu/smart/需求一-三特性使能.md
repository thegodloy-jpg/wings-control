...
# ⚠ 版本子串重叠：更具体型号行须排在更泛型号行之前（首个命中即返回）。
...
SMART_FEATURE_WHITELIST: tuple = (
    ...
    ("vllm",        ("qwen3.6","qwen3_6"),           ("*",),           frozenset({"spec","sparse"})),  # Qwen3.6 系列（同 qwen3_5_mtp）
    ("vllm",        ("glm-4.7",),                    ("*",),           frozenset({"spec","sparse","offload"})),
    ("vllm",        ("glm-5.1","glm5.1"),            ("*",),           frozenset({"sparse"})),  # 仅稀疏（NV/Ascend 一致，对齐 0430）；spec 不授→suffix 地板
    ("vllm",        ("glm-5","glm5"),                ("*",),           frozenset({"spec","sparse","offload"})),  # GLM-5 基座（5.1 在上、先匹配）
    ("vllm",        ("minimax-m2.7","minimax-m27"),  ("*",),           frozenset({"spec","sparse","offload"})),
    ("vllm",        ("deepseek-v4-flash","v4-flash"),("*",),           frozenset({"spec","sparse","offload"})),  # V4-Flash·NV day0（原 §6-④ 已补行）
    ...
    ("vllm_ascend", ("deepseek-v3.1","deepseek_v3.1","deepseek-r1","deepseek_r1"),
                                                     ("910b","910c"),  frozenset({"spec","offload"})),
...
```
> 卡型解析 `resolve_card_token` 兜底链已对齐引擎平台判定：`details[0].name` → `WINGS_DEVICE_NAME` → 平台信号(`WINGS_ASCEND_PLATFORM`/`ASCEND_PLATFORM`/`ENGINE_IMAGE_FLAVOR` → `ENGINE_VERSION` 后缀 a2/a3→910b/910c) → 显存(H20)。§2.3 产出口优先复用 C14 收口时 stash 的 `_smart_feats`（hardware_env 最准），避免双入口卡型不一致。
...
| 3 | vllm | glm-5.1 | * | sparse | `suffix`（白名单地板） | `indexcache_topk4` | off（白名单无+GLM51·NV 强制关） | **spec 未授→suffix（同行9）**；sparse=IndexCache |
...
> 三类标志性交互：**❗offload 抢占 spec**（2/5/7：mtp 被卸载降级为 suffix）；**✅V4 豁免共存**（8：native 连接器使 mtp 保留）；**白名单地板**（3/9：GLM-5.1 白名单仅 sparse、spec 不在表→suffix，修原误产 deepseek_mtp 的 bug；NV/Ascend 一致）。
...
> 行3（glm-5.1·NV）**不属本型**：与行9（glm-5.1·Ascend）一致，白名单仅 sparse、spec 未授→spec=`suffix`（地板）、sparse=`indexcache_topk4`、offload=off（不是"mtp 保留"）。
...
> 补充 : 问题一，算子加速的相关代码是不是没有完全删除， 第二个问题是@app.get("/v1/startup/accel")
...
这个调用的路径是不是有问题，第三个，日志的展示逻辑是不是需要增强，