...
[ -f /usr/local/Ascend/mindie/set_env.sh ] && { if command -v wings_source_env_with_diff >/dev/null 2>&1; then wings_source_env_with_diff /usr/local/Ascend/mindie/set_env.sh mindie/set_env.sh; else source /usr/local/Ascend/mindie/set_env.sh; fi; } || echo 'WARN: mindie/set_env.sh not found'
