...
def _build_monitor_proxy_proc(
    port_plan: PortPlan, env: dict, python_bin: str, uvicorn_mod: str
) -> "ManagedProc":
    """构建 monitor_proxy 进程的 ManagedProc 对象。"""
    return ManagedProc(
        name="monitor_proxy",
        argv=[
...
def _build_processes(port_plan: PortPlan) -> list[ManagedProc]:
    ...
    return [
        ...
        _build_monitor_proxy_proc(port_plan, env, python_bin, uvicorn_mod),
    ]
...
def _determine_role() -> str:
    ...
    # 两层判定一致。非 PD 的 Ray 分布式无 PD_ROLE → 此处不命中，行为字节级不变。
    try:
...
def _run_master_mode(
    ...
    # ---- 5. 守护循环 ----
    logger.info(
        "Master mode running: master_api=%d backend=%d proxy=%d health=%d monitor_proxy=%d",
        master_port,
...
def _launch_attempt(launch_args: LaunchArgs, port_plan: PortPlan, attempt_label: str) -> list:
    ...
    logger.info(
        "launcher running: backend=%s proxy=%s health=%s monitor_proxy=%s",
        port_plan.backend_port,
        port_plan.proxy_port,
        port_plan.health_port,
        _monitor_proxy_port(port_plan),
