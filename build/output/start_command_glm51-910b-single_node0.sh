...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "single", "hardware": "ascend", "nnodes": 1, "node_rank": 0, "distributed_backend": "ray", "tensor_parallel_size": 8, "model_name": "glm-5.1-32b-chat", "model_path": "D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_3hqmtciq", "backend_port": 17000}'
...
export VLLM_ASCEND_BALANCE_SCHEDULING=1
echo "[wings-env] export VLLM_ASCEND_BALANCE_SCHEDULING=${VLLM_ASCEND_BALANCE_SCHEDULING:-}"
echo '[wings-cmd] >>> exec python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 4096 --quantization ascend --seed 1024 --max-num-seqs 8 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --enable-chunked-prefill --enable-prefix-caching --additional-config '"'"'{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}'"'"' --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --host 192.168.1.100 --port 17000 --served-model-name glm-5.1-32b-chat --model D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_3hqmtciq --dtype auto --kv-cache-dtype auto --block-size 16 --default-chat-template-kwargs '"'"'{"enable_thinking":false}'"'"' --tensor-parallel-size 8'
python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 4096 --quantization ascend --seed 1024 --max-num-seqs 8 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --enable-chunked-prefill --enable-prefix-caching --additional-config '{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}' --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --host 192.168.1.100 --port 17000 --served-model-name glm-5.1-32b-chat --model D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_3hqmtciq --dtype auto --kv-cache-dtype auto --block-size 16 --default-chat-template-kwargs '{"enable_thinking":false}' --tensor-parallel-size 8 &
ENGINE_PID=$!
