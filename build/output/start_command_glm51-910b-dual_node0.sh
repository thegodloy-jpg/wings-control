...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "distributed", "hardware": "ascend", "nnodes": 2, "node_rank": 0, "distributed_backend": "dp_deployment", "tensor_parallel_size": 8, "model_name": "glm-5.1-32b-chat", "model_path": "D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_441qq0zo", "backend_port": 17000}'
...
echo "[wings-env] export HCCL_WHITELIST_DISABLE=${HCCL_WHITELIST_DISABLE:-}"
...
echo '[wings-cmd] >>> exec vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_441qq0zo --trust-remote-code --max-model-len 131072 --quantization ascend --seed 1024 --max-num-seqs 2 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --additional-config '"'"'{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}'"'"' --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --host 192.168.1.100 --port 17000 --served-model-name glm-5.1-32b-chat --dtype auto --kv-cache-dtype auto --block-size 16 --enable-expert-parallel --default-chat-template-kwargs '"'"'{"enable_thinking":false}'"'"' --tensor-parallel-size 8 --enable-prefix-caching --speculative-config '"'"'{"method": "deepseek_mtp", "num_speculative_tokens": 1}'"'"' --hf...<truncated>'
vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_441qq0zo --trust-remote-code --max-model-len 131072 --quantization ascend --seed 1024 --max-num-seqs 2 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --additional-config '{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}' --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --host 192.168.1.100 --port 17000 --served-model-name glm-5.1-32b-chat --dtype auto --kv-cache-dtype auto --block-size 16 --enable-expert-parallel --default-chat-template-kwargs '{"enable_thinking":false}' --tensor-parallel-size 8 --enable-prefix-caching --speculative-config '{"method": "deepseek_mtp", "num_speculative_tokens": 1}' --hf-overrides '{"index_topk_freq": 8}' --data-parallel-address 192.168.1.100 --data-parallel-rpc-port 13355 --data-parallel-size 2 --data-parallel-size-local 1 &
ENGINE_PID=$!
...
echo "[AdvFeature] Active advanced features: speculative_decode, sparse_kv"
if wait "$ENGINE_PID"; then
  ...
  echo "[AdvFeature] │ Features disabled: speculative_decode, sparse_kv"
  echo "[AdvFeature] │ Action: Restarting engine without advanced features"
  echo "[AdvFeature] └── Fallback command about to execute..."
  echo "[Engine] Falling back to basic mode (disabled: speculative_decode, sparse_kv)..."
...
export VLLM_ENGINE_READY_TIMEOUT_S=7200
echo "[wings-env] export VLLM_ENGINE_READY_TIMEOUT_S=${VLLM_ENGINE_READY_TIMEOUT_S:-}"
echo '[wings-cmd] >>> exec vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_441qq0zo --trust-remote-code --max-model-len 131072 --quantization ascend --seed 1024 --max-num-seqs 2 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --additional-config '"'"'{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}'"'"' --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --host 192.168.1.100 --port 17000 --served-model-name glm-5.1-32b-chat --dtype auto --kv-cache-dtype auto --block-size 16 --enable-expert-parallel --default-chat-template-kwargs '"'"'{"enable_thinking":false}'"'"' --tensor-parallel-size 8 --enable-prefix-caching --hf-overrides '"'"'{"index_topk_freq": 8}'"'"' --data-parallel-address 192.168.1.100 --dat...<truncated>'
vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_441qq0zo --trust-remote-code --max-model-len 131072 --quantization ascend --seed 1024 --max-num-seqs 2 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --additional-config '{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"ascend_compilation_config":{"enable_npugraph_ex":true}}' --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --host 192.168.1.100 --port 17000 --served-model-name glm-5.1-32b-chat --dtype auto --kv-cache-dtype auto --block-size 16 --enable-expert-parallel --default-chat-template-kwargs '{"enable_thinking":false}' --tensor-parallel-size 8 --enable-prefix-caching --hf-overrides '{"index_topk_freq": 8}' --data-parallel-address 192.168.1.100 --data-parallel-rpc-port 13355 --data-parallel-size 2 --data-parallel-size-local 1 &
ENGINE_PID=$!
echo "[Engine] Engine PID: $ENGINE_PID (advanced features disabled: speculative_decode, sparse_kv, fallback mode)"
