...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "single", "hardware": "ascend", "nnodes": 1, "node_rank": 0, "distributed_backend": "dp_deployment", "tensor_parallel_size": 16, "model_name": "DeepSeek-V4-Flash", "model_path": "D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_iqgzic7h", "backend_port": 17000}'
...
echo '[wings-cmd] >>> exec vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_iqgzic7h --trust-remote-code --max-model-len 65536 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --max-num-seqs 16 --tensor-parallel-size 4 --data-parallel-size 4 --enable-expert-parallel --quantization ascend --block-size 128 --chat-template /usr/local/serving/models/chat_template.jinja --async-scheduling --enable-prefix-caching --enable-chunked-prefill --safetensors-load-strategy prefetch --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --additional-config '"'"'{"enable_cpu_binding":true,"multistream_overlap_shared_expert":false,"multistream_dsa_preprocess":false,"ascend_compilation_config":{"enable_npugraph_ex":true,"enable_static_kernel":false}}'"'"' --tokenizer-mode deepsee...<truncated>'
vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_iqgzic7h --trust-remote-code --max-model-len 65536 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --max-num-seqs 16 --tensor-parallel-size 4 --data-parallel-size 4 --enable-expert-parallel --quantization ascend --block-size 128 --chat-template /usr/local/serving/models/chat_template.jinja --async-scheduling --enable-prefix-caching --enable-chunked-prefill --safetensors-load-strategy prefetch --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --additional-config '{"enable_cpu_binding":true,"multistream_overlap_shared_expert":false,"multistream_dsa_preprocess":false,"ascend_compilation_config":{"enable_npugraph_ex":true,"enable_static_kernel":false}}' --tokenizer-mode deepseek_v4 --tool-call-parser deepseek_v4 --enable-auto-tool-choice --host 192.168.1.100 --port 17000 --served-model-name DeepSeek-V4-Flash --dtype auto --kv-cache-dtype auto --seed 0 --default-chat-template-kwargs '{"thinking":false}' --api-server-count 1 --speculative-config '{"method": "deepseek_mtp", "num_speculative_tokens": 1}' &
ENGINE_PID=$!
