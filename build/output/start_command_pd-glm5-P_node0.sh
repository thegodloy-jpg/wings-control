...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "single", "hardware": "ascend", "nnodes": 1, "node_rank": 0, "distributed_backend": "ray", "tensor_parallel_size": 16, "model_name": "glm-5.1-chat", "model_path": "D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_o84jwzho", "backend_port": 17000}'
...
export VLLM_LLMDD_RPC_PORT=10521
...
(
  ...
  for i in $(seq 0 0); do
    ...
    ASCEND_RT_VISIBLE_DEVICES=$CARDS VLLM_MOONCAKE_BOOTSTRAP_PORT=$BOOTSTRAP python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 131072 --quantization ascend --seed 1024 --max-num-seqs 64 --max-num-batched-tokens 4096 --gpu-memory-utilization 0.95 --enable-chunked-prefill --additional-config '{"fuse_muls_add":true,"multistream_overlap_shared_expert":true,"recompute_scheduler_enable":true,"ascend_compilation_config":{"enable_npugraph_ex":true},"enable_dsa_cp":true}' --host 7.0.0.1 --served-model-name glm-5.1-chat --model D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_o84jwzho --dtype auto --kv-cache-dtype auto --block-size 16 --enable-expert-parallel --default-chat-template-kwargs '{"enable_thinking":false}' --kv-transfer-config '{"kv_connector":"MooncakeConnectorV1","kv_role":"kv_producer","kv_port":"'"$KVPORT"'","kv_connector_extra_config":{"prefill":{"dp_size":2,"tp_size":16},"decode":{"dp_size":16,"tp_size":4},"use_ascend_direct":true},"engine_id":"'"$RANK"'"}' --enable-auto-tool-choice --tool-call-parser glm47 --reasoning-parser glm45 --speculative-config '{"num_speculative_tokens":3,"method":"deepseek_mtp"}' --enforce-eager --port $PORT --tensor-parallel-size 16 --data-parallel-size 2 --data-parallel-rank $RANK --data-parallel-size-local 1 --data-parallel-address 7.0.0.1 --data-parallel-rpc-port 10521 --data-parallel-external-lb &
    pids+=($!)
  done
  wait -n || true
