...
ANALYZER_CONFIG='{"engine": "vllm", "deployment_mode": "single", "hardware": "nvidia", "nnodes": 1, "node_rank": 0, "distributed_backend": "mp", "tensor_parallel_size": 8, "model_name": "DeepSeek-V4-Flash", "model_path": "D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_lu1_3of0", "backend_port": 17000}'
...
FEATURES_EOF
  ...
  sleep 5
  ENGINE_START_EPOCH=$(date +%s)
echo '[wings-cmd] >>> exec python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 4096 --kv-cache-dtype fp8 --block-size 256 --enable-expert-parallel --tokenizer-mode deepseek_v4 --host 192.168.1.100 --port 17000 --served-model-name DeepSeek-V4-Flash --model D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_lu1_3of0 --dtype auto --gpu-memory-utilization 0.9 --max-num-batched-tokens 4096 --max-num-seqs 32 --seed 0 --default-chat-template-kwargs '"'"'{"thinking":false}'"'"' --tensor-parallel-size 8'
python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 4096 --kv-cache-dtype fp8 --block-size 256 --enable-expert-parallel --tokenizer-mode deepseek_v4 --host 192.168.1.100 --port 17000 --served-model-name DeepSeek-V4-Flash --model D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_lu1_3of0 --dtype auto --gpu-memory-utilization 0.9 --max-num-batched-tokens 4096 --max-num-seqs 32 --seed 0 --default-chat-template-kwargs '{"thinking":false}' --tensor-parallel-size 8 &
ENGINE_PID=$!
