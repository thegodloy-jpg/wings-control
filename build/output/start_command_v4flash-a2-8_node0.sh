...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "single", "hardware": "ascend", "nnodes": 1, "node_rank": 0, "distributed_backend": "dp_deployment", "tensor_parallel_size": 8, "model_name": "DeepSeek-V4-Flash", "model_path": "D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_v3vo1z3w", "backend_port": 17000}'
...
echo "[wings-env] export TRITON_ALL_BLOCKS_PARALLEL=${TRITON_ALL_BLOCKS_PARALLEL:-}"
export HCCL_OP_EXPANSION_MODE=${HCCL_OP_EXPANSION_MODE:-AIV}
echo "[wings-env] export HCCL_OP_EXPANSION_MODE=${HCCL_OP_EXPANSION_MODE:-}"
echo '[wings-cmd] >>> exec vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_v3vo1z3w --trust-remote-code --max-model-len 65536 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --max-num-seqs 16 --tensor-parallel-size 8 --data-parallel-size 1 --enable-expert-parallel --quantization ascend --block-size 128 --chat-template /usr/local/serving/models/chat_template.jinja --async-scheduling --enable-prefix-caching --enable-chunked-prefill --safetensors-load-strategy prefetch --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --additional-config '"'"'{"enable_cpu_binding":true,"multistream_overlap_shared_expert":false}'"'"' --tokenizer-mode deepseek_v4 --tool-call-parser deepseek_v4 --enable-auto-tool-choice --host 192.168.1.100 --port 17000 --served-model-name Deep...<truncated>'
vllm serve D:/project/inference/wings-control/wings-control-0730/wings-control/build/model_v3vo1z3w --trust-remote-code --max-model-len 65536 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --max-num-seqs 16 --tensor-parallel-size 8 --data-parallel-size 1 --enable-expert-parallel --quantization ascend --block-size 128 --chat-template /usr/local/serving/models/chat_template.jinja --async-scheduling --enable-prefix-caching --enable-chunked-prefill --safetensors-load-strategy prefetch --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --additional-config '{"enable_cpu_binding":true,"multistream_overlap_shared_expert":false}' --tokenizer-mode deepseek_v4 --tool-call-parser deepseek_v4 --enable-auto-tool-choice --host 192.168.1.100 --port 17000 --served-model-name DeepSeek-V4-Flash --dtype auto --kv-cache-dtype auto --seed 0 --default-chat-template-kwargs '{"thinking":false}' &
ENGINE_PID=$!
