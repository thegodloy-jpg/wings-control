#!/usr/bin/env bash
# =============================================================================
# 示例: vllm-ascend + Mooncake PD 分离 Prefill(P) 节点 start_command.sh
# 来源: wings_control.engines.vllm_adapter.build_start_script 生成逻辑
# 场景: Qwen3-8B, 单卡 TP=1, MooncakeConnectorV1, RDMA, P 节点 kv_producer
# =============================================================================
...
# PD / Mooncake 环境。P 节点为 kv_producer。
...
echo '[wings-cmd] >>> exec python3 -m vllm.entrypoints.openai.api_server --model '\''/models/Qwen3-8B'\'' --served-model-name Qwen3-8B --host 0.0.0.0 --port 17000 --tensor-parallel-size 1 --gpu-memory-utilization 0.9 --max-model-len 32768 --trust-remote-code --disable-log-requests --kv-transfer-config '\''{"kv_connector":"MooncakeConnectorV1","kv_role":"kv_producer","kv_connector_extra_config":{"mooncake_protocol":"rdma","prefill":{"tp_size":1,"dp_size":1,"pp_size":1},"decode":{"tp_size":1,"dp_size":1,"pp_size":1}}}'\'''
