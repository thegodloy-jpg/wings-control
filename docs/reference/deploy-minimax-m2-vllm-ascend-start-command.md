...
> 状态：历史启动命令分析。当前正式部署示例请以 [../deployment/docker-compose.md](../deployment/docker-compose.md)、[../deployment/k8s.md](../deployment/k8s.md) 和 [../features/index.md](../features/index.md) 为准；`wings_start.sh` 支持的启动项优先使用 CLI 字段，环境变量只保留无 CLI 的运行时变量。

> 适用范围：Wings Control 使用 `vllm_ascend` 引擎拉起 `MiniMax-M2.5` 或 `MiniMax-M2.7`。  
> 说明日期：2026-05-09。  
> 代码依据：`wings_control/core/config_loader.py`、`wings_control/engines/vllm_adapter.py`、`wings_control/config/recipes/architectures/minimax-m2.yaml`（Phase D 后承载体；旧 `config/defaults/ascend_default.json` 已于 2026-05-14 物理删除）。
