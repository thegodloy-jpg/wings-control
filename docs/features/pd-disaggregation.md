...
`wings_start.sh` 支持的模型、引擎、端口、设备数等启动项优先使用 CLI 字段；只有 PD 角色、RDMA 网卡、Mooncake 端口和引擎端口等无 CLI 字段的运行时项保留为环境变量。

### 3.1 通用 CLI 字段（P/D 共用）

```bash
--model-name "Qwen3-8B"
--model-path "/models/Qwen3-8B"
--engine vllm_ascend
--device-count 1                  # 每个节点使用的 NPU 数
--trust-remote-code
```

### 3.2 通用运行时环境变量（P/D 共用）
...
PORT=18200                        # Proxy 端口；正式示例中优先写成 --port 18200
...
PD_ROLE=D
ENGINE_PORT=17100
PORT=18100                        # Proxy 端口；正式示例中优先写成 --port 18100
HEALTH_PORT=19200
